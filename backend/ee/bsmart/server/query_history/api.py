...
from bsmart.auth.users import get_display_email
from bsmart.chat.chat_utils import create_chat_chain
from bsmart.configs.constants import MessageType
from bsmart.configs.constants import QAFeedbackType
from bsmart.configs.constants import SessionType
...
from ee.bsmart.db.query_history import fetch_chat_sessions_eagerly_by_time
...
def determine_flow_type(chat_session: ChatSession) -> SessionType:
    return (
        SessionType.SLACK
        if chat_session.bsmartbot_flow
...
def get_query_history_as_csv(
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": "attachment;filename=bsmart_query_history.csv"
        },
    )

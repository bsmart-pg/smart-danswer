...
def fetch_bsmartbot_analytics(
    ...
    Number of BsmartBot Queries (Chat Sessions)
    Number of instances of Negative feedback OR Needing additional help
        (only counting the last feedback)
    """
    # Get every chat session in the time range which is a Bsmartbot flow
    ...
    subquery_first_ai_response = (
        ...
        .where(
            ...
            ChatSession.bsmartbot_flow.is_(True),
        )
        .where(
    ...
    results = (
        db_session.query(
            ...
            # feedback aren't counted against Bsmartbot
            func.sum(
                case(
                    (
                        or_(
        ...
        # message of the chat session where the chat session is Bsmartbot type and within the time

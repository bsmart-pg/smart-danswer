...
from bsmart.configs.app_configs import POD_NAME
from bsmart.configs.app_configs import POD_NAMESPACE
from bsmart.configs.constants import BsmartRedisLocks
from bsmart.configs.constants import MessageType
from bsmart.configs.bsmartbot_configs import BSMART_BOT_REPHRASE_MESSAGE
from bsmart.configs.bsmartbot_configs import BSMART_BOT_RESPOND_EVERY_CHANNEL
from bsmart.configs.bsmartbot_configs import NOTIFY_SLACKBOT_NO_ANSWER
from bsmart.connectors.slack.utils import expert_info_from_slack_id
from bsmart.context.search.retrieval.search_runner import download_nltk_data
from bsmart.bsmartbot.slack.config import get_slack_channel_config_for_bot_and_channel
from bsmart.bsmartbot.slack.config import MAX_TENANTS_PER_POD
from bsmart.bsmartbot.slack.config import TENANT_ACQUISITION_INTERVAL
from bsmart.bsmartbot.slack.config import TENANT_HEARTBEAT_EXPIRATION
from bsmart.bsmartbot.slack.config import TENANT_HEARTBEAT_INTERVAL
from bsmart.bsmartbot.slack.config import TENANT_LOCK_EXPIRATION
from bsmart.bsmartbot.slack.constants import DISLIKE_BLOCK_ACTION_ID
from bsmart.bsmartbot.slack.constants import FEEDBACK_DOC_BUTTON_BLOCK_ACTION_ID
from bsmart.bsmartbot.slack.constants import FOLLOWUP_BUTTON_ACTION_ID
from bsmart.bsmartbot.slack.constants import FOLLOWUP_BUTTON_RESOLVED_ACTION_ID
from bsmart.bsmartbot.slack.constants import GENERATE_ANSWER_BUTTON_ACTION_ID
from bsmart.bsmartbot.slack.constants import IMMEDIATE_RESOLVED_BUTTON_ACTION_ID
from bsmart.bsmartbot.slack.constants import LIKE_BLOCK_ACTION_ID
from bsmart.bsmartbot.slack.constants import VIEW_DOC_FEEDBACK_ID
from bsmart.bsmartbot.slack.handlers.handle_buttons import handle_doc_feedback_button
from bsmart.bsmartbot.slack.handlers.handle_buttons import handle_followup_button
from bsmart.bsmartbot.slack.handlers.handle_buttons import (
    handle_followup_resolved_button,
)
from bsmart.bsmartbot.slack.handlers.handle_buttons import (
    handle_generate_answer_button,
)
from bsmart.bsmartbot.slack.handlers.handle_buttons import handle_slack_feedback
from bsmart.bsmartbot.slack.handlers.handle_message import handle_message
from bsmart.bsmartbot.slack.handlers.handle_message import (
    remove_scheduled_feedback_reminder,
)
from bsmart.bsmartbot.slack.handlers.handle_message import schedule_feedback_reminder
from bsmart.bsmartbot.slack.models import SlackMessageInfo
from bsmart.bsmartbot.slack.utils import check_message_limit
from bsmart.bsmartbot.slack.utils import decompose_action_id
from bsmart.bsmartbot.slack.utils import get_channel_name_from_id
from bsmart.bsmartbot.slack.utils import get_bsmart_bot_slack_bot_id
from bsmart.bsmartbot.slack.utils import read_slack_thread
from bsmart.bsmartbot.slack.utils import remove_bsmart_bot_tag
from bsmart.bsmartbot.slack.utils import rephrase_slack_message
from bsmart.bsmartbot.slack.utils import respond_in_thread
from bsmart.bsmartbot.slack.utils import TenantSocketModeClient
from bsmart.db.engine import get_all_tenant_ids
from bsmart.db.engine import get_session_with_tenant
from bsmart.db.models import SlackBot
from bsmart.db.search_settings import get_current_search_settings
from bsmart.db.slack_bot import fetch_slack_bots
from bsmart.key_value_store.interface import KvKeyNotFoundError
from bsmart.natural_language_processing.search_nlp_models import EmbeddingModel
from bsmart.natural_language_processing.search_nlp_models import warm_up_bi_encoder
from bsmart.one_shot_answer.models import ThreadMessage
from bsmart.redis.redis_pool import get_redis_client
from bsmart.server.manage.models import SlackBotTokens
...
class SlackbotHandler:
    ...
    def acquire_tenants(self) -> None:
        ...
        for tenant_id in tenant_ids:
            ...
            acquired = redis_client.set(
                BsmartRedisLocks.SLACK_BOT_LOCK,
                pod_id,
                nx=True,
    ...
    def send_heartbeats(self) -> None:
        ...
        for tenant_id in self.tenant_ids:
            redis_client = get_redis_client(tenant_id=tenant_id)
            heartbeat_key = (
                f"{BsmartRedisLocks.SLACK_BOT_HEARTBEAT_PREFIX}:{self.pod_id}"
            )
            redis_client.set(
    ...
    def shutdown(self, signum: int | None, frame: FrameType | None) -> None:
        ...
        for tenant_id in self.tenant_ids:
            try:
                redis_client = get_redis_client(tenant_id=tenant_id)
                redis_client.delete(BsmartRedisLocks.SLACK_BOT_LOCK)
...
def prefilter_requests(req: SocketModeRequest, client: TenantSocketModeClient) -> bool:
    ...
    if req.type == "events_api":
        ...
        if (
            ...
            or remove_bsmart_bot_tag(msg, client=client.web_client)
        ...
        bot_tag_id = get_bsmart_bot_slack_bot_id(client.web_client)
        if event_type == "message":
            is_dm = event.get("channel_type") == "im"
            is_tagged = bot_tag_id and bot_tag_id in msg
            is_bsmart_bot_msg = bot_tag_id and bot_tag_id in event.get("user", "")

            # BsmartBot should never respond to itself
            if is_bsmart_bot_msg:
                logger.info("Ignoring message from BsmartBot")
                return False

            # DMs with the bot don't pick up the @BsmartBot so we have to keep the
            # caught events_api
        ...
        if event.get("bot_profile"):
            ...
            # If BsmartBot is not specifically tagged and the channel is not set to respond to bots, ignore the message
        ...
        # Can respond in thread if it's an "im" directly to Bsmart or @BsmartBot is tagged
        if (
            thread_ts
    ...
    if req.type == "slash_commands":
        ...
        if not channel:
            channel_specific_logger.error(
                "Received BsmartBot command without channel - skipping"
            )
            return False
        ...
        if not sender:
            channel_specific_logger.error(
                "Cannot respond to BsmartBot command without sender to respond to."
            )
            return False
...
def build_request_details(
    ...
    if req.type == "events_api":
        ...
        msg = remove_bsmart_bot_tag(msg, client=client.web_client)

        if BSMART_BOT_REPHRASE_MESSAGE:
        ...
        if tagged:
            logger.debug("User tagged BsmartBot")
...
def process_message(
    ...
    client: TenantSocketModeClient,
    respond_every_channel: bool = BSMART_BOT_RESPOND_EVERY_CHANNEL,
    ...
    try:
        with get_session_with_tenant(client.tenant_id) as db_session:
            ...
            if (
                ...
                and not is_dm
                # If /BsmartBot (is_bot_msg) or @BsmartBot (bypass_filters)
...
def _get_socket_client(
    ...
    # https://docs.bsmart.dev/slack_bot_setup

...
from bsmart.configs.bsmartbot_configs import BSMART_BOT_FEEDBACK_REMINDER
from bsmart.configs.bsmartbot_configs import BSMART_REACT_EMOJI
from bsmart.bsmartbot.slack.blocks import get_feedback_reminder_blocks
from bsmart.bsmartbot.slack.handlers.handle_regular_answer import (
    handle_regular_answer,
)
from bsmart.bsmartbot.slack.handlers.handle_standard_answers import (
...
from bsmart.bsmartbot.slack.utils import fetch_user_ids_from_groups
from bsmart.bsmartbot.slack.utils import respond_in_thread
from bsmart.bsmartbot.slack.utils import slack_usage_report
...
from bsmart.db.users import add_slack_user_if_not_exists
...
def schedule_feedback_reminder(
    ...
    if not BSMART_BOT_FEEDBACK_REMINDER:
    ...
    future = now + datetime.timedelta(minutes=BSMART_BOT_FEEDBACK_REMINDER)
...
def handle_message(
    ...
    Bsmart failing to answer/retrieve docs does count and should be notified
    """
    ...
    if respond_tag_only and not bypass_filters:
        logger.info(
            ...
            "BsmartBot only responds to tags"
        )
        return False
    ...
    # If configured to respond to team members only, then cannot be used with a /BsmartBot command
    ...
    if send_to and is_bot_msg:
        if sender_id:
            respond_in_thread(
                ...
                text="The BsmartBot slash command is not enabled for this channel",
                thread_ts=None,
            )

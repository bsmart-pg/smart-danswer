...
from bsmart.configs.constants import BSMART_METADATA_FILENAME
from bsmart.file_processing.html_utils import parse_html_page_basic
...
# To include additional metadata in the search index, add a .bsmart_metadata.json file
...
def load_files_from_zip(
    ...
    with zipfile.ZipFile(zip_file_io, "r") as zip_file:
        zip_metadata = {}
        try:
            metadata_file_info = zip_file.getinfo(BSMART_METADATA_FILENAME)
            with zip_file.open(metadata_file_info, "r") as metadata_file:
                try:
                    ...
                    logger.warn(f"Unable to load {BSMART_METADATA_FILENAME}")
        except KeyError:
            logger.info(f"No {BSMART_METADATA_FILENAME} file")
        ...
        for file_info in zip_file.infolist():
            with zip_file.open(file_info.filename, "r") as file:
                ...
                ) or file_info.filename == BSMART_METADATA_FILENAME:
                    continue
                yield file_info, file, zip_metadata.get(file_info.filename, {})


def _extract_bsmart_metadata(line: str) -> dict | None:
    html_comment_pattern = r"<!--\s*BSMART_METADATA=\{(.*?)\}\s*-->"
    hashtag_pattern = r"#BSMART_METADATA=\{(.*?)\}"
...
def read_text_file(
    ...
    encoding: str = "utf-8",
    errors: str = "replace",
    ignore_bsmart_metadata: bool = True,
) -> tuple[str, dict]:
    metadata = {}
    file_content_raw = ""
    for ind, line in enumerate(file):
        ...
        if ind == 0:
            metadata_or_none = (
                None if ignore_bsmart_metadata else _extract_bsmart_metadata(line)
            )

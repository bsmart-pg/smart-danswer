...
from bsmart.server.bsmart_api.ingestion import api_key_dep
...
PUBLIC_ENDPOINT_SPECS = [
    ...
    ("/auth/type", {"GET"}),
    # just gets the version of Bsmart (e.g. 0.3.11)
    ("/version", {"GET"}),
...
def check_router_auth(
    ...
    control_plane_dep = fetch_ee_implementation_or_noop(
        "bsmart.server.tenants.access", "control_plane_dep"
    )
    current_cloud_superuser = fetch_ee_implementation_or_noop(
        "bsmart.auth.users", "current_cloud_superuser"
    )

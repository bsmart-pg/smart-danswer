...
from bsmart.auth.api_key import get_hashed_api_key_from_request
...
from bsmart.configs.app_configs import DISABLE_AUTH
from bsmart.configs.app_configs import DISABLE_VERIFICATION
from bsmart.configs.app_configs import EMAIL_FROM
from bsmart.configs.app_configs import REQUIRE_EMAIL_VERIFICATION
from bsmart.configs.app_configs import SESSION_EXPIRE_TIME_SECONDS
from bsmart.configs.app_configs import SMTP_PASS
from bsmart.configs.app_configs import SMTP_PORT
from bsmart.configs.app_configs import SMTP_SERVER
from bsmart.configs.app_configs import SMTP_USER
from bsmart.configs.app_configs import TRACK_EXTERNAL_IDP_EXPIRY
...
from bsmart.configs.constants import BSMART_API_KEY_DUMMY_EMAIL_DOMAIN
from bsmart.configs.constants import BSMART_API_KEY_PREFIX
from bsmart.configs.constants import UNNAMED_KEY_PLACEHOLDER
from bsmart.db.api_key import fetch_user_for_api_key
from bsmart.db.auth import get_access_token_db
from bsmart.db.auth import get_default_admin_user_emails
from bsmart.db.auth import get_user_count
from bsmart.db.auth import get_user_db
from bsmart.db.auth import SQLAlchemyUserAdminDB
from bsmart.db.engine import get_async_session
from bsmart.db.engine import get_async_session_with_tenant
from bsmart.db.engine import get_session_with_tenant
from bsmart.db.models import AccessToken
from bsmart.db.models import OAuthAccount
...
def get_display_email(email: str | None, space_less: bool = False) -> str:
    if email and email.endswith(BSMART_API_KEY_DUMMY_EMAIL_DOMAIN):
        name = email.split("@")[0]
        if name == BSMART_API_KEY_PREFIX + UNNAMED_KEY_PLACEHOLDER:
...
def send_user_verification_email(
    ...
    msg = MIMEMultipart()
    msg["Subject"] = "Bsmart Email Verification"
    msg["To"] = user_email
    if mail_from:
        msg["From"] = mail_from
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def create(
        ...
        tenant_id = await fetch_ee_implementation_or_noop(
            "bsmart.server.tenants.provisioning",
...
async def optional_user(
    ...
    versioned_fetch_user = fetch_versioned_implementation(
        "bsmart.auth.users", "optional_user_"
    )
...
def get_default_admin_user_emails_() -> list[str]:
    # No default seeding available for Bsmart MIT
    return []
...
def create_bsmart_oauth_router(

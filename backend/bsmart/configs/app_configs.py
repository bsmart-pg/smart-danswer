...
from bsmart.configs.constants import DocumentIndexType
from bsmart.file_processing.enums import HtmlBasedConnectorTransformLinksStrategy
...
# NOTE: if you are having problems accessing the Bsmart web UI locally (especially
...
# restrict access to Bsmart to only users with emails from those domains.
# E.g. `VALID_EMAIL_DOMAINS=example.com,example.org` will restrict Bsmart
...
# If set, Bsmart will listen to the `expires_at` returned by the identity
...
# DB Configs
#####
DOCUMENT_INDEX_NAME = "bsmart_index"
...
VESPA_DEPLOYMENT_ZIP = (
    os.environ.get("VESPA_DEPLOYMENT_ZIP") or "/app/bsmart/vespa-app.zip"
)
...
# View the list here:
# https://github.com/bsmart-ai/bsmart/blob/main/backend/bsmart/connectors/factory.py
...
# Logs Bsmart only model interactions like prompts, responses, messages etc.
LOG_BSMART_MODEL_INTERACTIONS = (
    os.environ.get("LOG_BSMART_MODEL_INTERACTIONS", "").lower() == "true"
)
...
# founders@bsmart.ai OR message Chris Weaver or Yuhong Sun in the Bsmart
# Slack community (https://join.slack.com/t/bsmart/shared_invite/zt-1w76msxmd-HJHLe3KNFIAIzk_0dSOKaQ)

...
from bsmart.access.access import get_access_for_document
from bsmart.background.celery.apps.app_base import task_logger
from bsmart.background.celery.celery_redis import celery_get_queue_length
from bsmart.background.celery.tasks.shared.RetryDocumentIndex import RetryDocumentIndex
from bsmart.background.celery.tasks.shared.tasks import LIGHT_SOFT_TIME_LIMIT
from bsmart.background.celery.tasks.shared.tasks import LIGHT_TIME_LIMIT
from bsmart.configs.app_configs import JOB_TIMEOUT
from bsmart.configs.constants import CELERY_VESPA_SYNC_BEAT_LOCK_TIMEOUT
from bsmart.configs.constants import BsmartCeleryQueues
from bsmart.configs.constants import BsmartRedisLocks
from bsmart.db.connector import fetch_connector_by_id
from bsmart.db.connector import mark_cc_pair_as_permissions_synced
from bsmart.db.connector import mark_ccpair_as_pruned
from bsmart.db.connector_credential_pair import add_deletion_failure_message
from bsmart.db.connector_credential_pair import (
...
from bsmart.db.document import count_documents_by_needs_sync
from bsmart.db.document import get_document
from bsmart.db.document import get_document_ids_for_connector_credential_pair
from bsmart.db.document import mark_document_as_synced
from bsmart.db.document_set import delete_document_set
from bsmart.db.document_set import delete_document_set_cc_pair_relationship__no_commit
from bsmart.db.document_set import fetch_document_sets
from bsmart.db.document_set import fetch_document_sets_for_document
from bsmart.db.document_set import get_document_set_by_id
from bsmart.db.document_set import mark_document_set_as_synced
from bsmart.db.engine import get_session_with_tenant
from bsmart.db.enums import IndexingStatus
from bsmart.db.index_attempt import delete_index_attempts
from bsmart.db.index_attempt import get_index_attempt
from bsmart.db.index_attempt import mark_attempt_failed
from bsmart.db.models import DocumentSet
...
from bsmart.document_index.interfaces import VespaDocumentFields
from bsmart.redis.redis_connector import RedisConnector
from bsmart.redis.redis_connector_credential_pair import RedisConnectorCredentialPair
from bsmart.redis.redis_connector_delete import RedisConnectorDelete
from bsmart.redis.redis_connector_doc_perm_sync import RedisConnectorPermissionSync
from bsmart.redis.redis_connector_doc_perm_sync import (
    RedisConnectorPermissionSyncPayload,
)
from bsmart.redis.redis_connector_index import RedisConnectorIndex
from bsmart.redis.redis_connector_prune import RedisConnectorPrune
from bsmart.redis.redis_document_set import RedisDocumentSet
from bsmart.redis.redis_pool import get_redis_client
from bsmart.redis.redis_usergroup import RedisUserGroup
...
from bsmart.utils.variable_functionality import (
    fetch_versioned_implementation_with_fallback,
)
from bsmart.utils.variable_functionality import global_version
from bsmart.utils.variable_functionality import noop_fallback
...
def check_for_vespa_sync_task(self: Task, *, tenant_id: str | None) -> None:
    ...
    lock_beat = r.lock(
        BsmartRedisLocks.CHECK_VESPA_SYNC_BEAT_LOCK,
        ...
        if global_version.is_ee_version():
            try:
                fetch_user_groups = fetch_versioned_implementation(
                    "bsmart.db.user_group", "fetch_user_groups"
                )
...
def try_generate_user_group_sync_tasks(
    ...
    fetch_user_group = fetch_versioned_implementation(
        "bsmart.db.user_group", "fetch_user_group"
    )
...
def monitor_vespa_sync(self: Task, tenant_id: str | None) -> bool:
    ...
    lock_beat: RedisLock = r.lock(
        BsmartRedisLocks.MONITOR_VESPA_SYNC_BEAT_LOCK,
        timeout=CELERY_VESPA_SYNC_BEAT_LOCK_TIMEOUT,
    )
    ...
    try:
        ...
        n_indexing = celery_get_queue_length(
            BsmartCeleryQueues.CONNECTOR_INDEXING, r_celery
        )
        n_sync = celery_get_queue_length(
            BsmartCeleryQueues.VESPA_METADATA_SYNC, r_celery
        )
        n_deletion = celery_get_queue_length(
            BsmartCeleryQueues.CONNECTOR_DELETION, r_celery
        )
        n_pruning = celery_get_queue_length(
            BsmartCeleryQueues.CONNECTOR_PRUNING, r_celery
        )
        n_permissions_sync = celery_get_queue_length(
            BsmartCeleryQueues.CONNECTOR_DOC_PERMISSIONS_SYNC, r_celery
        )
        ...
        for key_bytes in r.scan_iter(RedisUserGroup.FENCE_PREFIX + "*"):
            ...
            monitor_usergroup_taskset = fetch_versioned_implementation_with_fallback(
                "bsmart.background.celery.tasks.vespa.tasks",
        ...
        # length = celery_get_queue_length(BsmartCeleryQueues.VESPA_METADATA_SYNC, r_celery)
        # task_logger.warning(f"queue={BsmartCeleryQueues.VESPA_METADATA_SYNC} length={length}")
    except SoftTimeLimitExceeded:
        task_logger.info(
            "Soft time limit exceeded, task is being terminated gracefully."
        )

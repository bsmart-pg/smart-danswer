...
from bsmart.background.celery.celery_utils import extract_ids_from_runnable_connector
from bsmart.background.celery.tasks.indexing.tasks import IndexingCallback
from bsmart.configs.app_configs import ALLOW_SIMULTANEOUS_PRUNING
from bsmart.configs.app_configs import JOB_TIMEOUT
from bsmart.configs.constants import CELERY_PRUNING_LOCK_TIMEOUT
...
from bsmart.connectors.factory import instantiate_connector
...
from bsmart.db.document import get_documents_for_connector_credential_pair
...
from bsmart.utils.logger import pruning_ctx
...
def check_for_pruning(self: Task, *, tenant_id: str | None) -> None:
    ...
    lock_beat = r.lock(
        BsmartRedisLocks.CHECK_PRUNE_BEAT_LOCK,
...
def try_creating_prune_generator_task(
    ...
    lock = r.lock(
        BSMART_REDIS_FUNCTION_LOCK_PREFIX + "try_creating_prune_generator_task",
    ...
    try:
        ...
        celery_app.send_task(
            ...
            queue=BsmartCeleryQueues.CONNECTOR_PRUNING,
            task_id=custom_task_id,
            priority=BsmartCeleryPriority.LOW,
        )
...
def connector_pruning_generator_task(
    ...
    lock = r.lock(
        BsmartRedisLocks.PRUNING_LOCK_PREFIX + f"_{redis_connector.id}",

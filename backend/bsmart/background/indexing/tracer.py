...
class BsmartTracer:
    ...
    def start(self) -> None:
        tracemalloc.start(BSMART_TRACEMALLOC_FRAMES)
    ...
    def log_previous_diff(self, numEntries: int) -> None:
        ...
        BsmartTracer.log_diff(self.snapshot, self.snapshot_prev, numEntries)

    def log_first_diff(self, numEntries: int) -> None:
        if not self.snapshot or not self.snapshot_first:
            return

        BsmartTracer.log_diff(self.snapshot, self.snapshot_first, numEntries)

...
from bsmart.chat.models import BsmartAnswer
...
from bsmart.configs.chat_configs import QUOTE_ALLOWED_ERROR_PERCENT
from bsmart.context.search.models import InferenceChunk
from bsmart.prompts.constants import ANSWER_PAT
from bsmart.prompts.constants import QUOTE_PAT
from bsmart.utils.logger import setup_logger
from bsmart.utils.text_processing import clean_model_quote
from bsmart.utils.text_processing import clean_up_code_blocks
from bsmart.utils.text_processing import extract_embedded_json
from bsmart.utils.text_processing import shared_precompare_cleanup
...
def match_quotes_to_docs(
    ...
    bsmart_quotes: list[BsmartQuote] = []
    for quote in quotes:
    ...
    return BsmartQuotes(quotes=bsmart_quotes)
...
def _process_answer(
...
) -> tuple[BsmartAnswer, BsmartQuotes]:
    ...
    if not answer:
        ...
        return BsmartAnswer(answer=None), BsmartQuotes(quotes=[])

    logger.notice(f"Answer: {answer}")
    if not quote_strings:
        logger.debug("No quotes extracted from raw output")
        return BsmartAnswer(answer=answer), BsmartQuotes(quotes=[])
    ...
    return BsmartAnswer(answer=answer), quotes
...
class QuotesProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[BsmartAnswerPiece | BsmartQuotes, None, None]:
        # None -> end of stream
        if token is None:
            ...
            if m:
                ...
                for quote_idx in quote_indices:
                    ...
                    if num_backslashes % 2 == 0:
                        yield BsmartAnswerPiece(answer_piece=remaining[:quote_idx])
                        return

                # If no unescaped quote found, yield the remaining string
                if len(remaining) > 0:
                    yield BsmartAnswerPiece(answer_piece=remaining)
                return
            ...
            if self.is_json_prompt and _stream_json_answer_end(model_previous, token):
                ...
                if token:
                    try:
                        ...
                        yield BsmartAnswerPiece(
                            answer_piece=self.hold_quote + token[:answer_token_section]
                        )
                    except ValueError:
                        logger.error("Quotation mark not found in token")
                        yield BsmartAnswerPiece(answer_piece=self.hold_quote + token)
                yield BsmartAnswerPiece(answer_piece=None)
                return

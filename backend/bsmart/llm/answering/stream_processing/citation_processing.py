...
from bsmart.configs.chat_configs import STOP_STREAM_PAT
from bsmart.llm.answering.stream_processing.utils import DocumentIdOrderMapping
from bsmart.prompts.constants import TRIPLE_BACKTICK
...
class CitationProcessor:
    ...
    def process_token(
        self, token: str | None
    ) -> Generator[BsmartAnswerPiece | CitationInfo, None, None]:
        # None -> end of stream
        if token is None:
            yield BsmartAnswerPiece(answer_piece=self.curr_segment)
            return
        ...
        if result:
            yield BsmartAnswerPiece(answer_piece=result)

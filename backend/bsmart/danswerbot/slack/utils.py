...
from bsmart.configs.app_configs import DISABLE_TELEMETRY
from bsmart.configs.constants import ID_SEPARATOR
from bsmart.configs.constants import MessageType
from bsmart.configs.bsmartbot_configs import BSMART_BOT_FEEDBACK_VISIBILITY
from bsmart.configs.bsmartbot_configs import BSMART_BOT_MAX_QPM
from bsmart.configs.bsmartbot_configs import BSMART_BOT_MAX_WAIT_TIME
from bsmart.configs.bsmartbot_configs import BSMART_BOT_NUM_RETRIES
from bsmart.configs.bsmartbot_configs import (
    BSMART_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD,
)
from bsmart.configs.bsmartbot_configs import (
    BSMART_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS,
)
from bsmart.connectors.slack.utils import make_slack_api_rate_limited
from bsmart.connectors.slack.utils import SlackTextCleaner
from bsmart.bsmartbot.slack.constants import FeedbackVisibility
from bsmart.db.engine import get_session_with_tenant
from bsmart.db.users import get_user_by_email
from bsmart.llm.exceptions import GenAIDisabledException
from bsmart.llm.factory import get_default_llms
...
from bsmart.prompts.miscellaneous_prompts import SLACK_LANGUAGE_REPHRASE_PROMPT
...
from bsmart.utils.text_processing import replace_whitespaces_w_space

logger = setup_logger()


_BSMART_BOT_SLACK_BOT_ID: str | None = None
_BSMART_BOT_MESSAGE_COUNT: int = 0
_BSMART_BOT_COUNT_START_TIME: float = time.time()


def get_bsmart_bot_slack_bot_id(web_client: WebClient) -> Any:
    global _BSMART_BOT_SLACK_BOT_ID
    if _BSMART_BOT_SLACK_BOT_ID is None:
        _BSMART_BOT_SLACK_BOT_ID = web_client.auth_test().get("user_id")
    return _BSMART_BOT_SLACK_BOT_ID
...
def check_message_limit() -> bool:
    ...
    """
    if BSMART_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD == 0:
        return True
    global _BSMART_BOT_MESSAGE_COUNT
    global _BSMART_BOT_COUNT_START_TIME
    time_since_start = time.time() - _BSMART_BOT_COUNT_START_TIME
    if time_since_start > BSMART_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS:
        _BSMART_BOT_MESSAGE_COUNT = 0
        _BSMART_BOT_COUNT_START_TIME = time.time()
    if (_BSMART_BOT_MESSAGE_COUNT + 1) > BSMART_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD:
        logger.error(
            f"BsmartBot has reached the message limit {BSMART_BOT_RESPONSE_LIMIT_PER_TIME_PERIOD}"
            f" for the time period {BSMART_BOT_RESPONSE_LIMIT_TIME_PERIOD_SECONDS} seconds."
            " These limits are configurable in backend/bsmart/configs/bsmartbot_configs.py"
        )
        return False
    _BSMART_BOT_MESSAGE_COUNT += 1
    return True
...
def remove_bsmart_bot_tag(message_str: str, client: WebClient) -> str:
    bot_tag_id = get_bsmart_bot_slack_bot_id(web_client=client)
    return re.sub(rf"<@{bot_tag_id}>\s", "", message_str)


@retry(
    tries=BSMART_BOT_NUM_RETRIES,
    delay=0.25,
    backoff=2,
...
def read_slack_thread(
    ...
    for reply in replies:
        if "user" in reply and "bot_id" not in reply:
            ...
            self_slack_bot_id = get_bsmart_bot_slack_bot_id(client)
            ...
            if reply.get("user") == self_slack_bot_id:
                ...
                # BsmartBot responses have both text and blocks
                ...
                if not blocks:
                    logger.warning(f"BsmartBot response has no blocks: {reply}")
                    continue
                ...
                if message.startswith("_Filters"):
                    if len(blocks) < 2:
                        ...
                        continue
                    # This is the BsmartBot answer format, if there is a change to how we respond,
                    # this will need to be updated to get the correct "answer" portion
                    message = reply["blocks"][1].get("text", {}).get("text")
            else:
                # Other bots are not counted as the LLM response which only comes from Bsmart
                message_type = MessageType.USER
        ...
        message = remove_bsmart_bot_tag(message, client=client)
...
def slack_usage_report(
    ...
    if sender_email is not None:
        with get_session_with_tenant(tenant_id) as db_session:
            bsmart_user = get_user_by_email(email=sender_email, db_session=db_session)

    optional_telemetry(
        record_type=RecordType.USAGE,
        data={"action": action},
        user_id=str(bsmart_user.id) if bsmart_user else "Non-Bsmart-Or-No-Auth-User",
    )


class SlackRateLimiter:
    def __init__(self) -> None:
        self.max_qpm: int | None = BSMART_BOT_MAX_QPM
        self.max_wait_time = BSMART_BOT_MAX_WAIT_TIME
...
def get_feedback_visibility() -> FeedbackVisibility:
    try:
        return FeedbackVisibility(BSMART_BOT_FEEDBACK_VISIBILITY.lower())
    except ValueError:

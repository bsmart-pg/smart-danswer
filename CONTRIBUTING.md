<!-- BSMART_METADATA={"link": "https://github.com/bsmart-ai/bsmart/blob/main/CONTRIBUTING.md"} -->
...
To run the backend API server, navigate back to `bsmart/backend` and run:
```bash
...
Then, from the `bsmart/backend` directory, run:
```bash
pre-commit install
```
